 ...
 * Main Process
 */
int CPemMaker::makePemFile(QString *p12Path, QString *passwd, QString *number)
{
    int i;
    int pos1=0, pos2=0;
    QString no; //社員番号
    ...
    if ((fp = fopen(p12Path->toStdString().c_str(), "rb")) == nullptr) {
    ...
    PKCS12_free(p12);
    if ((fp = fopen(pemFileName.toStdString().c_str(), "w+")) == nullptr) {
    ...
    if (ca != nullptr && sk_X509_num(ca) > 0) {
